...
const SavedPetitionsCard: React.FC<SavedPetitionsCardProps> = ({
  petitions,
  onViewAll
}) => {
  ...
  return <>
      <Card>
        ...
        <CardContent className="space-y-3">
          {petitions.slice(0, 3).map(petition => <div key={petition.id} className="p-3 bg-gray-50 rounded-lg">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 text-sm text-gray-600">
                    <span className="flex items-center gap-1">
                      ...
                      {format(new Date(petition.created_at), "dd 'de' MMM", {
                    locale: ptBR
                  })}
                    </span>
                  </div>
                </div>
                <Button variant="ghost" size="sm" onClick={() => handleViewPetition(petition)} className="ml-2">
                  <Eye className="w-4 h-4" />
                </Button>
              </div>
            </div>)}
          {petitions.length > 3 && <p className="text-sm text-gray-500 text-center pt-2">
              +{petitions.length - 3} petições adicionais
            </p>}
        </CardContent>
      </Card>

      <VisualizarPeticao peticao={selectedPetition} isOpen={isDialogOpen} onClose={closeDialog} />
    </>;
};
