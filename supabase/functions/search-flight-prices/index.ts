...
// Função para obter token de acesso da Amadeus (AMBIENTE DE TESTE)
const getAmadeusToken = async (): Promise<string> => {
  ...
  console.log('🔑 Verificando credenciais Amadeus...');
  console.log(`Client ID presente: ${clientId ? 'SIM' : 'NÃO'}`);
  console.log(`Client Secret presente: ${clientSecret ? 'SIM' : 'NÃO'}`);
  ...
  console.log(`🌐 Usando ambiente de TESTE: ${tokenUrl}`);
  ...
  try {
    ...
    console.log(`📡 Status da resposta do token: ${response.status} ${response.statusText}`);

    if (!response.ok) {
      const errorText = await response.text();
      console.error('❌ Erro detalhado ao obter token Amadeus:', errorText);
...
const searchFlights = async (params: FlightSearchParams): Promise<{ 
  ...
  console.log('✈️ Buscando voos para:', params);
  ...
  try {
    ...
    console.log(`🗺️ Códigos IATA: ${originCode} -> ${destinationCode}`);
    
    // Usando API de TESTE
    const searchUrl = 'https://test.api.amadeus.com/v2/shopping/flight-offers';
    console.log(`🌐 Usando API de busca de TESTE: ${searchUrl}`);
    ...
    console.log(`📡 Status da busca de voos: ${response.status} ${response.statusText}`);

    if (!response.ok) {
      const errorText = await response.text();
      console.error('❌ Erro na busca de voos:', errorText);
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🎉 Busca de voos concluída com sucesso!');
